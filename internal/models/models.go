...
type Order struct {
	...
	CustomerID    int       `json:"custumer_id"`
...
// Status is the type for order statues
type Status struct {
	ID        int       `json:"id"`
	Name      string    `json:"name"`
	CreatedAt time.Time `json:"-"`
...
// InsertTransaction insertes a new txn, and returns its id
	...
	stmt := `
		INSERT INTO transactions
			(amount, currency, last_four, bank_return_code, transaction_status_id, 
				payment_intent, payment_method, expire_month, expire_year)
		VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)
	`
...
// InsertOrder insertes a new order, and returns its id
